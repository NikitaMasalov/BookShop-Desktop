...
class ProductsMenu:
    ...
    def initialize_menu(self):
        ...
        product_name_label = tk.Label(self.entry_frame, text='Название книги:', bg=my_config.BACKGROUND)
        product_name_label.grid(row=0, column=0, sticky=tk.E)
        product_price_label = tk.Label(self.entry_frame, text='Стоимость:', bg=my_config.BACKGROUND)
        product_price_label.grid(row=1, column=0, sticky=tk.E)
        in_stock_label = tk.Label(self.entry_frame, text='На складе:', bg=my_config.BACKGROUND)
        in_stock_label.grid(row=2, column=0, sticky=tk.E)
        description_label = tk.Label(self.entry_frame, text='описание(необязательный):',
...
class OrdersMenu:
    ...
    def add_order(self):
        ...
        # function itself check if there is enough products, and count total price (quantity*price)
            ...
            self.error_message("недостаточное количество продукции на утилизации")
